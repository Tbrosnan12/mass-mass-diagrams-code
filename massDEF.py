...
def P_dot(mp,mc):
    Pq1 = ((-192*np.pi)/(5*(1+ap*ac)))*((G_*np.pi/(c**3*P*(60*60*24)))**(5/3))*f*(mp*mc/((mp+mc)**(1/3)))
    
    Pq2=((-32*np.pi)/(5*(1+ap*ac)))*((G_*np.pi/(c**3*P*(60*60*24)))**(5/3))*f*(mp*mc/((mp+mc)**(1/3)))*(ap+ac-(ap*mp+ac*mc)/(mp+mc))**2
    
    Pm= ((-3*np.pi)/(1+ap*ac))*((G_*np.pi/(c**3*P*(60*60*24)))**(5/3))*(e**2*(1+e**2/4)/((1-e**2)**(7/2)))*(mp*mc/((mp+mc)**(1/3)))*((5/3)*(ap+ac)-(2/3)*(ap*mp+ac*mc)/(mp+mc)+(Bp*ac+Bc*ap)/(1+ap*ac))
    
    return Pq1+Pq2+Pm
   
def w_dot(mp,mc):
    
    w = 3*(G_*(mp+mc)/c**3)**(2/3)*(2*np.pi/(P*(60*60*24)))**(5/3)/(1-e**2)*((1-(1/3)*ap*ac)/(1+ap*ac)-(mp*Bc*(ap**2)+mc*Bp*(ac**2))/(6*(mp+mc)*((1+ap*ac)**2)))
